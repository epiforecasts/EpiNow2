// Profiles update_Rt_ref() (the reference), the plain Stan rewrite and the
// C++ update_Rt() declared in inst/stan/functions/rt.stan in one model.
functions {
#include update_rt_ref.stan
#include update_rt_rewrite.stan
#include rt.stan
}
data {
  int t;
...
generated quantities {
  real maxdiff_rewrite = max(abs(
    update_Rt_ref(t, R0, noise, bps, bp_effects, stationary, n_centre) -
      update_Rt_rewrite(t, R0, noise, bps, bp_effects, stationary, n_centre)
  ));
