...
transformed parameters {
  ...
  // The delay PMFs below are deterministic and, for fixed distributions,
  // structurally constant. Computing them in a local block keeps rstan from
  // monitoring them, avoiding a spurious NA R-hat. They are recomputed where
    ...
    if (estimate_r) {
      ...
      profile("infections") {
        real fraction_observed = get_param(
          param_id_fraction_observed, params_fixed_lookup,
        ...
        infections = generate_infections(
          ...
          pop_floor, future_time, obs_scale, fraction_observed, 1
        );
      }
    } else {
      // via deconvolution
...
generated quantities {
  ...
  profile("generated quantities") {
    ...
    {
      // Recompute the delay PMFs kept local in transformed parameters.
      vector[delay_np_pmf_length] delay_np_pmf_use = combine_np_pmf(
        delay_np_pmf, delay_n_np_est, delay_np_est_groups,
        delay_np_est_pos, delay_np_est_raw
      );
