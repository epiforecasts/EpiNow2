...
transformed data{
  // model
  int estimate_r = (prior_model > 0 ? 1 : 0); // estimate time_varying r/R
  int R_gp = (prior_model == 1); // gp on R
  int gr_gp = (prior_model == 2); // gp on growth rate
  // observations
...
transformed parameters {
  ...
  vector<upper = 10 * r_mean>[estimate_r > 0 ? ot_h : 0] R; // reproduction number / growth rate
  ...
  vector[estimate_r] base_r;                                // log_R (if GP on reproduction number), or base growth rate (if GP on growth)
  ...
  if (estimate_r) {
    if (R_gp) {
      base_r[estimate_r] = log_R[estimate_r];
    } else {
      base_r = R_to_growth(log_R, gt_mean[estimate_r], gt_sd[estimate_r]);
    }
    // via Rt
    R = update_Rt(R, base_r[estimate_r], noise, breakpoints, bp_effects, stationary, R_gp);
    infections = generate_infections(R, seeding_time, gt_mean, gt_sd, max_gt,
                                     initial_infections, initial_growth,
                                     pop, future_time, R_gp);
  }else{
    // via deconvolution
...
generated quantities {
  ...
  vector[R_gp ? 0 : ot_h] gen_R;
  vector[prior_model == 2 ? 0 : ot_h] r;
  vector[ot] log_lik;
  if (R_gp){
