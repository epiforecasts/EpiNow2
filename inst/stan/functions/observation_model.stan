...
// update log density for reported cases
  ...
  if (obs_dist) {
    real dispersion = inv_square(phi_sd > 0 ? rep_phi[obs_dist] : phi_mean);
    if (phi_sd > 0) {
...
vector report_log_lik(array[] int cases, vector reports,
                      array[] real rep_phi, int obs_dist, real weight) {
  int t = num_elements(reports);
  vector[t] log_lik;
  ...
  if (obs_dist == 0) {
    ...
    real dispersion = inv_square(rep_phi[obs_dist]);
    for (i in 1:t) {
...
array[] int report_rng(vector reports, array[] real rep_phi, int obs_dist) {
  int t = num_elements(reports);
  array[t] int sampled_reports;
  real dispersion = 1e5;
  if (obs_dist) {
    dispersion = inv_square(rep_phi[obs_dist]);
  }
