...
vector convolve_pmfs(vector pmfs, int[] mdelay, int delays) {
  ...
  conv_pmf[1:mdelay[1]] = head(pmfs, mdelay[1]);
  if (delays > 1) {
    for (s in 2:delays) {
      ...
      proc_pmf = rep_vector(0, cdelay);
      for (z in 1:cdelay) {
        for (x in 1:mdelay[s]){
          if (z - x >= 0) { 
            proc_pmf[z] += pmfs[sum(mdelay[s - 1]) + x] * 
                           cpmf[z - x + 1];
          }
        }
      }
...
vector static_pmf(vector dmean, vector dsd, int[] dmax, int dists, int reverse) {
  ...
  pmf = convolve_pmfs(pmf, dmax, dists, reverse);
  if (reverse) {
