...
vector combine_pmfs(vector fixed_pmf, real[] pmf_mu, real[] pmf_sigma, int[] pmf_n, int[] dist, int len, int zeroes, int reverse_pmf) {
  ...
  if (zeroes == 1 || zeroes == 2) { // truncate or shift
    real norm;
    if (zeroes == 2) { // shift
      for (i in 2:len) {
        pmf[len - i + 2] = pmf[len - i + 1];
      }
    }
    pmf[1] = 0;
    norm = sum(pmf);
