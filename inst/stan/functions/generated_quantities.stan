...
int[] report_rng(vector reports, real[] rep_phi, int model_type) {
  ...
  if (model_type) {
    ...
    for (s in 1:t) {
      ...
      if (sqrt_phi > large_phi) {
        sampled_reports[s] = poisson_rng(reports[s] > large ? large : reports[s]);
      } else {
        sampled_reports[s] = neg_binomial_2_rng(reports[s] > large ? large : reports[s], sqrt_phi);
      }
    }
  }else {
    for (s in 1:t) {
