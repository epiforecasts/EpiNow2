...
\begin{equation}
\overline{\mathrm{GP}} = \frac{1}{T_\mathrm{obs}} \sum_{t = 1}^{T_\mathrm{obs}} \sum_{i = 1}^{t - 1} \mathrm{GP}_i
\end{equation}

the empirical mean of the cumulative path over the observation window, we sample $\overline{\log R}$ (the mean of $\log R_t$ over the observation window) and form
...
\begin{equation}
\frac{1}{T_\mathrm{obs}} \sum_{t = 1}^{T_\mathrm{obs}} \log R_t = \overline{\log R}
\end{equation}
...
\begin{equation}
\log R_1 = \overline{\log R} - \overline{\mathrm{GP}}
\end{equation}

via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). The transformation from $(\overline{\log R}, \mathrm{GP}_1, \dots, \mathrm{GP}_{T_\mathrm{obs} - 1})$ to $(\log R_1, \mathrm{GP}_1, \dots, \mathrm{GP}_{T_\mathrm{obs} - 1})$ is a pure shift, so its Jacobian determinant is one. The user's prior is a density on $R_1$ on the natural scale, so the only Jacobian term required is $\log R_1$, for the transform between $R_1$ and $\log R_1$.
