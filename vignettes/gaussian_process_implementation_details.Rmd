...
via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). With the GP increments held fixed, the transformation from $\overline{\log R}$ to $\log R_1$ is a pure shift by $-\overline{\mathrm{GP}}$, so its Jacobian is one. The user's prior is a density on $R_1$ on the natural scale, so the only Jacobian term required is $\log R_1$, for the transform between $R_1$ and $\log R_1$.
