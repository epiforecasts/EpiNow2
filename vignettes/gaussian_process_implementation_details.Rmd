...
The standard fix in regression is to centre the predictors so the intercept becomes the response value at the mean of the predictors rather than at zero, which decorrelates the intercept from the slope and gives the sampler a cleaner geometry (this is how [`brms`](https://CRAN.R-project.org/package=brms) parameterises intercepts internally, for example). We apply the same idea to the cumulative GP path: the internal "intercept" is the mean of $\log R_t$ over the observation window — well-identified by the data — and the cumulative path is centred about that mean. With $\overline{\mathrm{GP}} = \frac{1}{T_\mathrm{obs}} \sum_{t = 1}^{T_\mathrm{obs}} \sum_{i = 1}^{t - 1} \mathrm{GP}_i$ the empirical mean of the cumulative path over the observation window, we sample $\log \overline{R}$ (the mean of $\log R_t$ over the window) and form
