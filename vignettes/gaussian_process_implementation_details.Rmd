...
In order to make our models computationally tractable, we approximate the Gaussian Process using a Hilbert space approximation to the Gaussian Process [@approxGP], centred around mean zero.

\begin{equation}
\mathrm{GP}(0, k(\Delta t)) \approx \sum_{j=1}^m \left(S_k(\sqrt{\lambda_j}) \right)^\frac{1}{2} \phi_j(t) \beta_j
\end{equation}

with $m$ the number of basis functions to use in the approximation, and values of $\lambda_j$ given by
...
# Choosing the boundary and number of basis functions

The accuracy of the approximation depends on the boundary $L$ and the number of basis functions $m$ relative to the lengthscale.
We write $L = cS$ with $c$ the boundary factor and $S$ the half-range of the time points, and $\ell = \rho / S$ for the lengthscale relative to the half-range.
For the Matérn 3/2 kernel, @approxGP (Section 4.3.1) show that the approximation is accurate when

\begin{align}
c &\geq \max(1.2, 4.5 \ell) \\
m &\geq 3.42 \frac{c}{\ell}
\end{align}

with constants $(3.2, 1.75)$ for the squared exponential kernel and $(4.1, 2.65)$ for the Matérn 5/2 kernel.
Longer lengthscales therefore need a wider boundary, and for a given lengthscale no number of basis functions compensates for a boundary that is too narrow.
Shorter lengthscales need more basis functions.

By default, `EpiNow2` sets $c$ from the 95% quantile of the lengthscale prior and then $m$ from the 5% quantile, so that lengthscales across the bulk of the prior are approximated accurately.
The Matérn 3/2 constants are used for the Ornstein-Uhlenbeck kernel, which @approxGP do not cover.
The periodic kernel does not use a boundary, and uses $m = 0.2 t_\mathrm{GP}$ by default, where $t_\mathrm{GP}$ is the number of time points to which the Gaussian Process is applied.

After fitting, `EpiNow2` applies the lengthscale diagnostic of @approxGP (Section 4.5).
A warning is given if the posterior median lengthscale is shorter than the $3.42 c S / m$ days that $m$ basis functions can represent, or longer than the $c S / 4.5$ days that the boundary supports.
Either can be addressed by adjusting the lengthscale prior, from which the settings are chosen, or by setting `basis_prop` and `boundary_scale` in `gp_opts()` directly.

# Modelling the reproduction number

The default `estimate_infections()` model uses [the GP to model the first-order difference of $\log R_t$](estimate_infections.html#time-varying-reproduction-number), with the trajectory itself built by cumulative summation of GP values $\mathrm{GP}_i$. Writing $T_\mathrm{obs}$ for the length of the observation window and $R_1$ for the initial reproduction number, this construction is
...
This parallels a common issue in standard regression problems, where the intercept and the slope can trade off against each other unless the predictors are centred. There, a common fix is to [centre the predictors](https://mc-stan.org/docs/stan-users-guide/efficiency-tuning.html#standardizing-predictors) so the intercept becomes the response value at the mean of the predictors rather than at zero, which decorrelates the intercept from the slope and gives the sampler a cleaner geometry; this is how, for example, [`brms`](https://paulbuerkner.com/brms/reference/brmsformula.html#parameterization-of-the-population-level-intercept) parameterises intercepts internally. Here the cumulative GP increments play the role of the slope (they determine the drift of $\log R_t$) and $\log R_1$ plays the role of the intercept.
...
via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). Stan explores $\overline{\log R}$ on the unconstrained scale, so the prior placed there must carry the Jacobian of the map to $R_1$,
...
The internal parameter is declared as a positive real $\overline{R} = \exp(\overline{\log R})$, so Stan supplies $\log \overline{R}$ itself as the Jacobian of its own log transform. The correction added by hand is therefore the difference

\begin{equation}
\log R_1 - \log \overline{R} = - \overline{\mathrm{GP}}.
\end{equation}

A breakpoint random walk, when used, is centred over the observation window in the same way and enters $\log R_t$ as a further additive term. Its contribution to the offset between $\log R_1$ and $\overline{\log R}$ adds to $-\overline{\mathrm{GP}}$, and since it too is a deterministic function of the other parameters that does not involve $\overline{\log R}$, the change of variables and its Jacobian are unchanged.
