...
via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). With the GP increments held fixed, the Jacobian of this transform from $\overline{\log R}$ to $R_1$ is $|dR_1 / d\overline{\log R}| = R_1$, so the log-Jacobian correction added to the target is $\log R_1$.
