...
To remove the ridge while preserving the user-facing interpretation of the prior on $R_1$, we sample the trajectory mean over the observation window as the internal parameter and apply the user's prior to the derived initial value. With $\overline{\mathrm{GP}} = \frac{1}{T_\mathrm{obs}} \sum_{t = 1}^{T_\mathrm{obs}} \sum_{i = 1}^{t - 1} \mathrm{GP}_i$ the empirical mean of the cumulative path over the observation window, we sample $\log \overline{R}$ (the mean of $\log R_t$ over the observation window) and form
...
via a change of variables. The transformation from $(\log \overline{R}, \mathrm{GP}_1, \dots, \mathrm{GP}_{T_\mathrm{obs} - 1})$ to $(\log R_1, \mathrm{GP}_1, \dots, \mathrm{GP}_{T_\mathrm{obs} - 1})$ is a pure shift, so its Jacobian determinant is one and the only Jacobian term required is the one for the log–natural-scale transform of $R_1$ itself. In practice the prior contribution is the user-specified density evaluated at $R_1$, plus $\log R_1$ when the prior was specified on the natural scale (lognormal, gamma, truncated normal).
