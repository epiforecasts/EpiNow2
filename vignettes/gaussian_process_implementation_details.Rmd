...
Without further constraint this parameterisation makes inference difficult: a constant added to $\log R_1$ and subtracted from the cumulative GP path leaves $\log R_t$ unchanged for all $t$, so $R_1$ and the long-run drift of the GP are not jointly identified by the data. The likelihood is flat along this direction (a ridge in the joint posterior), giving the sampler poor geometry and, on some random seeds, stuck chains and large $\hat R$ values.
...
via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). The Jacobian of this transform is
...
The same identifiability argument applies to any cumulative time-varying contribution to $\log R_t$. When breakpoints are used (see `rt_opts(breakpoints = ...)` and the [breakpoint random walk option](estimate_infections.html#time-varying-reproduction-number)), the cumulative path of breakpoint effects produces an analogous ridge in the joint posterior with $\log R_1$. The breakpoint path is therefore centred over the observation window in the same way as the cumulative GP path above, alongside (or instead of) it, so that $\overline{\log R}$ remains the well-identified internal parameter regardless of which time-varying components are active.
