...
the empirical mean of the cumulative path over the observation window (each $\mathrm{GP}_i$ enters the cumulative sum for every $t \geq i + 1$, hence the linearly decreasing weight in the second form), we sample $\overline{\log R}$ (the mean of $\log R_t$ over the observation window) and form
...
\begin{equation}
\overline{\log R} = \frac{1}{T_\mathrm{obs}} \sum_{t = 1}^{T_\mathrm{obs}} \log R_t
\end{equation}

by construction, breaking the ridge. Centring only over the observation window (not the forecast horizon) ensures that the fitted historical $R_t$ does not shift when the forecast horizon is changed.
...
via a [change of variables](https://mc-stan.org/docs/stan-users-guide/reparameterization.html#changes-of-variables). With the GP increments held fixed, the Jacobian of this transform from $\overline{\log R}$ to $R_1$ is

\begin{equation}
\left| \frac{d R_1}{d \overline{\log R}} \right| = R_1
\end{equation}

so the log-Jacobian correction added to the target is $\log R_1$.
