...
% ==========================================
% Internal components — 2x2 grid
\node[intbox] (rt) at (-1.4, -2.7)
  {Reproduction number (Rt)};
\node[intbox] (infmod) at (1.4, -2.7)
  {Infection model\\[-1pt]
    {\tiny renewal / backcalculation}};
\node[intbox] (delays) at (-1.4, -3.7)
  {Delay distributions\\[-1pt]
    {\tiny convolution}};
\node[intbox] (obsmod) at (1.4, -3.7)
  {Observation model\\[-1pt]
    {\tiny NB / Poisson}};

% Main function label at bottom of block
\node[estbox, minimum width=4cm] (estinf)
  at (0, -4.8) {estimate\_infections()};
...
% Truncation distribution feeds into block
\draw[darr] (esttrunc.south)
  -- ++(0, -0.4)
  -| (infbg.north east)
  node[lblstyle, pos=0.55, right, xshift=1pt]
    {truncation dist.};

% ==========================================
%  OPTIONS — LEFT SIDE
% ==========================================
\node[optbox] at (-5.0, -2.5) (rtopts)  {rt\_opts()};
\node[optbox] at (-5.0, -3.1) (gpopts)  {gp\_opts()};
\node[optbox] at (-5.0, -3.7) (delayopts)
  {delay\_opts()};
\node[optbox] at (-5.0, -4.3) (backopts)
  {backcalc\_opts()};

% Arrows from left options to block boundary
\draw[optarr] (rtopts.east)  -- (infbg.west |- rtopts);
\draw[optarr] (gpopts.east)  -- (infbg.west |- gpopts);
...
%  OPTIONS — RIGHT SIDE
% ==========================================
\node[optbox] at (5.0, -2.5) (gtopts)   {gt\_opts()};
\node[optbox] at (5.0, -3.1) (obsopts)  {obs\_opts()};
\node[optbox] at (5.0, -3.7) (truncopts)
  {trunc\_opts()};
...
\node[databox, left=1.2cm of estsec,
  minimum width=2.2cm]
...
% Secondary options
\node[optbox, right=1.0cm of estsec]
  (secopts) {secondary\_opts()};
\node[optbox, below=0.2cm of secopts]
  (secdelay) {delay\_opts()};
\node[optbox, below=0.2cm of secdelay]
  (secobs) {obs\_opts()};
