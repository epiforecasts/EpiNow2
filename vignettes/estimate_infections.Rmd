...
The model is initialised before the first initerved data point by assuming constant exponential growth for the mean of modelled delays from infection to case report (called `seeding_time` $t_\mathrm{seed}$ in the model):
...
\begin{align}
  ...
  I_\mathrm{init} &= C_\mathrm{init} / \xi
\end{align}
...
\begin{equation}
log(C_t) = \log(C_\mathrm{init}) + r_\mathrm{init} t + \epsilon_t
\end{equation}
where $C_{t}$ is the number of reported cases on day $t$, $C_\mathrm{init}$ an estimated intercept, and $\epsilon_{t}$ the error term.
