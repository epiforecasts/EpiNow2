...
This vignette benchmarks available model options in `{EpiNow2}` and compares the models based on their speed (run time) and accuracy of estimates relative to data with known trajectories. Thoughout, we compare various models to the default model as it is the first model most users encounter.
...
#> DEBUG [2024-08-07 14:41:13] epinow: Running in exact mode for 2000 samples (across 4 chains each with a warm up of 250 iterations each) and 114 time steps of which 0 are a forecast
```
...
#> DEBUG [2024-08-07 14:41:54] simulate_infections: Running in exact mode for 1 samples (across 1 chains each with a warm up of 1 iterations each) and 154 time steps of which 40 are a forecast
```
...
Note that we will run the model using 133 observations so that we can compare the forecast performance over the full set of observations.
...
|:-------------------|:----------------------------------------------------------------------------------------------------------------------|------------------------:|
|default_vb_rstan    |Default model; Variational algorithm for sampling instead of MCMC and using [`{rstan}`]                                |                     3.7s|
|non_mechanistic     |Gaussian process on new infection and with no mechanistic process on $R_t$                                             |                    17.6s|
|default_vb_cmdstanr |Default model; Variational algorithm for sampling instead of MCMC and using [`{cmdstanr}`]                             |                    29.1s|
|no_rw_gp            |Gaussian process on new infection and with a stationary Gaussian process prior on $R_t$                                |   156.6s (~2.61 minutes)|
|rw7_no_gp           |No Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary process           |   163.2s (~2.72 minutes)|
|default_mcmc_rstan  |Default model; Gaussian process on new infection and with a non-stationary prior on $R_t$                              |   461.6s (~7.69 minutes)|
|rw7_gp              |Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary Gaussian process     |     786s (~13.1 minutes)|
|rw1_no_gp           |No Gaussian process on new infections, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process |  831.8s (~13.86 minutes)|
|rw1_gp              |Gaussian process on new infection, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process     | 1018.9s (~16.98 minutes)|
...
Table: Run times and $R_t$ estimation/forecast performance - measured by total CRPS - of various _EpiNow2_ models (ordered by run times)

|model               |                  runtime| estimate| estimate based on partial data| forecast|description                                                                                                            |
|:-------------------|------------------------:|--------:|------------------------------:|--------:|:----------------------------------------------------------------------------------------------------------------------|
|default_vb_rstan    |                     3.7s|       52|                           1.74|     0.77|Default model; Variational algorithm for sampling instead of MCMC and using [`{rstan}`]                                |
|non_mechanistic     |                    17.6s|       13|                           0.38|     0.81|Gaussian process on new infection and with no mechanistic process on $R_t$                                             |
|default_vb_cmdstanr |                    29.1s|       50|                           1.54|     0.56|Default model; Variational algorithm for sampling instead of MCMC and using [`{cmdstanr}`]                             |
|no_rw_gp            |   156.6s (~2.61 minutes)|       12|                           2.65|     3.15|Gaussian process on new infection and with a stationary Gaussian process prior on $R_t$                                |
|rw7_no_gp           |   163.2s (~2.72 minutes)|       15|                           1.46|     0.97|No Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary process           |
|default_mcmc_rstan  |   461.6s (~7.69 minutes)|       12|                           2.50|     3.07|Default model; Gaussian process on new infection and with a non-stationary prior on $R_t$                              |
|rw7_gp              |     786s (~13.1 minutes)|       12|                           2.28|     2.48|Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary Gaussian process     |
|rw1_no_gp           |  831.8s (~13.86 minutes)|       12|                           1.09|     0.92|No Gaussian process on new infections, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process |
|rw1_gp              | 1018.9s (~16.98 minutes)|       12|                           1.57|     1.31|Gaussian process on new infection, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process     |
...
![](speedup_options-infections_plot-1.png)
...
Table: Run times and infection estimation/forecast performance - measured by total CRPS - of various _EpiNow2_ models (ordered by run times)

|model               |                  runtime|        estimate| estimate based on partial data|       forecast|description                                                                                                            |
|:-------------------|------------------------:|---------------:|------------------------------:|--------------:|:----------------------------------------------------------------------------------------------------------------------|
|default_vb_rstan    |                     3.7s| 100,466,540,936|                 13,939,986,486| 15,073,685,594|Default model; Variational algorithm for sampling instead of MCMC and using [`{rstan}`]                                |
|non_mechanistic     |                    17.6s| 105,382,366,655|                 18,389,520,217| 22,702,833,489|Gaussian process on new infection and with no mechanistic process on $R_t$                                             |
|default_vb_cmdstanr |                    29.1s| 101,219,563,072|                 14,719,133,605| 16,492,003,842|Default model; Variational algorithm for sampling instead of MCMC and using [`{cmdstanr}`]                             |
|no_rw_gp            |   156.6s (~2.61 minutes)| 101,325,993,238|                 16,067,861,076| 11,657,638,342|Gaussian process on new infection and with a stationary Gaussian process prior on $R_t$                                |
|rw7_no_gp           |   163.2s (~2.72 minutes)| 100,610,962,704|                 16,463,137,139| 15,899,825,962|No Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary process           |
|default_mcmc_rstan  |   461.6s (~7.69 minutes)| 100,739,492,931|                 15,385,085,099| 11,950,312,064|Default model; Gaussian process on new infection and with a non-stationary prior on $R_t$                              |
|rw7_gp              |     786s (~13.1 minutes)| 100,477,354,094|                 15,424,644,624| 11,612,080,016|Gaussian process on new infection, and $R_t$ modelled with 7-day random walk and a non-stationary Gaussian process     |
|rw1_no_gp           |  831.8s (~13.86 minutes)| 101,999,831,970|                 16,692,997,044| 16,186,550,701|No Gaussian process on new infections, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process |
|rw1_gp              | 1018.9s (~16.98 minutes)| 100,856,003,921|                 16,181,287,681| 14,139,769,277|Gaussian process on new infection, and $R_t$ modelled with 1-day random walk and a non-stationary Gaussian process     |
...
The fastest model was `default_vb_rstan`, which was over 124.8 times faster than the default model. Comparatively, it was worse at estimating $R_t$ on complete data, better than the default model in estimating $R_t$ with partial data, and better at forecasting it. For estimating and forecasting infections, this model was better at estimating it using complete data, better with partial data, and worse at forecasting it.

The non-mechanistic model is different to the other models in that it does not model $R_t$ mechanistically. It was over 26.2 times faster than the default model. Compared to the default model, it was worse at estimating $R_t$ on complete data, better than the default model in estimating $R_t$ on partial data, and better at forecasting it. For estimating and forecasting infections, this model was worse at estimating using complete, worse with partial data and worse at forecasting.

Finally, let's compare the slowest model to the default. The slowest model was `rw1_gp`. It was 2.2 times slower than the default model. Compared to the default model, it was better at estimating $R_t$ on complete data, better on estimating with partial data, and better at forecasting $R_t$. In terms of estimating and forecasting infections, the  model was worse at estimating with complete data, and additionally, worse in estimating with partial data, and worse at forecasting.
...
Another thing to note is that here, we used 4 cores for the simulations and so using more or fewer cores might change the run time results. We, however, expect the relative rankings to be the same or similar. To speed up the model runs, we recommend checking the number of cores available on your machine using `parallel::detectCores()` and passing a high enough number of cores to `mc.cores` through the `options()` function. See the benchmarking data setup chunk above for an example.
