...
| Component | Model | Function | Parameter | Default | Syntax Example |
|-----------|-------|----------|-----------|---------|----------------|
| **Reproduction number (Rt)** |
| Initial R₀ | Renewal only | `rt_opts()` | `prior` | `LogNormal(mean=1, sd=1)` | `rt_opts(prior = LogNormal(mean=2, sd=0.5))` |
| Random walk SD | Renewal only | `rt_opts()` | (auto) | `HalfNormal(0, 0.1)` | Set via `rw` option: `rt_opts(rw=7)` |
| **Gaussian Process** |
| Length scale | Both | `gp_opts()` | `ls` | `LogNormal(mean=21, sd=7, max=60)` | `gp_opts(ls = LogNormal(mean=14, sd=5, max=30))` |
| Magnitude | Both | `gp_opts()` | `alpha` | `Normal(mean=0, sd=0.01)` | `gp_opts(alpha = Normal(mean=0, sd=0.05))` |
| **Back-calculation** |
| Prior source | Non-mechanistic only | `backcalc_opts()` | `prior` | `"reports"` | `backcalc_opts(prior = "infections")` |
| **Observation model** |
| Overdispersion | Both | `obs_opts()` | `dispersion` | `Normal(mean=0, sd=0.25)` | `obs_opts(dispersion = Normal(mean=0, sd=0.5))` |
| Scaling | Both | `obs_opts()` | `scale` | `Fixed(1)` | `obs_opts(scale = Normal(mean=0.5, sd=0.1))` |
| Day of week | Both | `obs_opts()` | (auto) | `Dirichlet(1,...,1)` | Turn off via `week_effect=FALSE` |
| **Generation time** |
| Parameters | Renewal only | `gt_opts()` | `dist` | Fixed at 1 day | `gt_opts(Gamma(shape=Normal(3,1), rate=Normal(2,0.5)))` |
| **Delays** |
| Parameters | Both | `delay_opts()` | `dist` | Fixed at 0 days | `delay_opts(LogNormal(mean=5, sd=2, max=14))` |
| **Truncation** |
| Distribution | Both | `trunc_opts()` | `dist` | Fixed at 0 days | `trunc_opts(LogNormal(mean=0.5, sd=0.5, max=3))` |
| **Initialization** |
| Initial infections | Both | (internal) | `iota` | `Normal(log(C_init), 2)` | Not user-adjustable |

## estimate_secondary() priors

| Component | Function | Parameter | Default | Syntax Example |
|-----------|----------|-----------|---------|----------------|
| **Delays (primary to secondary)** |
| Parameters | `delay_opts()` | `dist` | `LogNormal(meanlog=Normal(2.5,0.5), sdlog=Normal(0.47,0.25), max=30)` | `delay_opts(LogNormal(mean=3, sd=2, max=20))` |
| **Observation model** |
| Scaling | `obs_opts()` | `scale` | Required, no default | `obs_opts(scale = Normal(mean=0.4, sd=0.1))` |
| Overdispersion | `obs_opts()` | `dispersion` | `Normal(mean=0, sd=0.25)` | `obs_opts(dispersion = Normal(mean=0, sd=0.5))` |
| Day of week | `obs_opts()` | (auto) | `Dirichlet(1,...,1)` | Turn off via `week_effect=FALSE` |
...
| Parameters | `trunc_opts()` | `dist` | `LogNormal(meanlog=Normal(0,1), sdlog=Normal(1,1), max=10)` | `trunc_opts(LogNormal(meanlog=Normal(0.5,0.5), sdlog=Normal(0.5,0.5), max=7))` |
| **Observation model** |
...
## Initial reproduction number (R₀)

**What it controls:** The starting value of the time-varying reproduction number at the beginning of your observation period.
...
- **Higher mean**: Shifts initial estimates upward. Use for fast-spreading pathogens when you observe rapid initial growth.
...
- **`weight_prior` option**: Default `TRUE` weights the prior by data length, keeping parameters near their priors. Set `FALSE` for short time series where you want the model to update the generation time.
...
- **Set `weight_prior = FALSE`** only for short time series with very uncertain generation time
...
**Expected impact:**
- **Uncertain parameters**: Allow model to learn delay from data, but require
longer time series (>30 days)
- **Fixed parameters**: Faster, more stable with short time series

**When to modify:**
- Use literature estimates when available (e.g., case-to-death ~14-21 days
for COVID-19)
