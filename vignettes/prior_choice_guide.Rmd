...
**Key principle:** All defaults in `{EpiNow2}` aim to provide sensible defaults that work across a range of pathogens and settings. For a specific application with domain knowledge, you may benefit from more informative priors.
...
We load the _EpiNow2_ package and the _posterior_ and _data.table_ packages which we will use for convergence diagnostics and data wrangling, respectively.
...
#> The following objects are masked from 'package:EpiNow2':
#> 
#>     sd, var
...
| Initial / mean Rt | Renewal only | `rt_opts()` | `prior` | `GP(init = LogNormal(mean = 1, sd = 1))` | Initial value (`GP(init=)`) or mean (`GP(mean=)`) of the reproduction number |
| **Random walk** |
| Step size SD | Renewal only | `RW()` | `sd` | `Normal(0, 0.1)` | Random-walk step sd; interval set with `period` |
| **Gaussian Process** |
| Length scale | Both | `GP()` | `ls` | `LogNormal(mean = 21, sd = 7, max = 60)` | Controls smoothness over time |
| Magnitude | Both | `GP()` | `alpha` | `Normal(mean = 0, sd = 0.01)` | Controls amplitude of variations |
| **Back-calculation** |
| Latent infections | Non-mechanistic only | `backcalc_opts()` | `prior` | `GP(init = ...)` | Gaussian process on log-infections |
| **Observation model** |
...
| **Observation model** |
| Scaling | `obs_opts()` | `scale` | `Fixed(1)` | Fraction of primary events leading to secondary |
...
- **GP on first differences** (`GP(init = ...)`, the default): This is the prior for $R$ at the start of the time series. When there is no data (e.g., early in the time series before the seeding period ends, or when doing real-time estimates of delayed quantities), $R_t$ reverts to the last estimated value but with expanding uncertainty as the zero-centered GP of differences continues.
- **Mean-reverting GP** (`GP(mean = ...)`): This is the prior mean that $R_t$ reverts to throughout the time series when there is no data (e.g., early in the time series, or near the present when there are delays). Useful when you want estimates of $R_t$ to return to a baseline value in the absence of information.
...
- **Higher mean**: Shifts the initial $R_0$ estimate upward (differenced) or provides a higher reversion point (stationary). Use for fast-spreading pathogens based on prior knowledge from the literature.
...
- You're analysing a well-characterised pathogen (e.g., influenza pandemic with $R_0$ typically 1.5-2.5, or emerging outbreaks of measles in immunologically naive populations)
...
``` r
# For a measles outbreak with strong prior knowledge on the initial value
rt_opts(prior = GP(init = LogNormal(mean = 15, sd = 2)))
#> $use_rt
#> [1] TRUE
#> 
#> $rw
...
#> 
#> $prior
#> Time-varying parameter: Gaussian process (on first differences)
#> - initial-value prior:
...
# For an uncertain but likely growing outbreak
...
# Mean-reverting (stationary) GP reverting to R_t = 2 when there is no data
rt_opts(prior = GP(mean = LogNormal(mean = 2, sd = 0.5)))
#> $use_rt
#> [1] TRUE
#> 
#> $rw
...
#> 
#> $prior
#> Time-varying parameter: Gaussian process (mean-reverting)
#> - mean prior:
#> - lognormal distribution:
#>   meanlog:
#>     0.66
...
GP(init = LogNormal(mean = 1, sd = 1), ls = LogNormal(mean = 7, sd = 3, max = 30))
#> Time-varying parameter: Gaussian process (on first differences)
#> - initial-value prior:
#> - lognormal distribution:
#>   meanlog:
...
GP(init = LogNormal(mean = 1, sd = 1), ls = LogNormal(mean = 28, sd = 10, max = 90))
#> Time-varying parameter: Gaussian process (on first differences)
#> - initial-value prior:
#> - lognormal distribution:
#>   meanlog:
...
**What it controls:** The amplitude of variations in the Gaussian Process.

- **For renewal equation model**: This controls variations in $\log(R_t)$ and is the prior standard deviation around the GP trend (stationary GP) or around zero (differenced GP).
- **For non-mechanistic model**: This controls variations in log infections directly, and typically requires a less constrained prior since it scales with the infection level rather than $R_t$.

**Default:** `Normal(mean = 0, sd = 0.01)`

For the renewal equation model, this corresponds to a half-normal (since negative values are truncated) with 95% of prior mass below 0.02, reflecting small changes in $\log(R_t)$.

**Expected impact of changes:**

- **Larger `sd` (e.g., 0.05-0.1)**: Allows bigger jumps over time. Useful for outbreaks with large, real variations in transmission (e.g., due to major interventions) or for non-mechanistic models. May overfit if too large.
- **Smaller `sd` (e.g., 0.005)**: Constrains to change very gradually. Good for stable situations or very noisy data. May underfit real changes if too small.

**When to modify:**

- **Larger**: Modelling outbreaks with major interventions (lockdowns, vaccination campaigns), high real variation, or using the non-mechanistic model
- **Smaller**: Modelling stable endemic transmission or when data is very noisy relative to sample size
- **Note**: The default works well for the renewal equation model in most scenarios, but may need adjustment for the non-mechanistic model

**Example:**

``` r
# For outbreak with major interventions
GP(init = LogNormal(mean = 1, sd = 1), alpha = Normal(mean = 0, sd = 0.05))
#> Time-varying parameter: Gaussian process (on first differences)
#> - initial-value prior:
#> - lognormal distribution:
#>   meanlog:
...
GP(init = LogNormal(mean = 1, sd = 1), alpha = Normal(mean = 0, sd = 0.005))
#> Time-varying parameter: Gaussian process (on first differences)
#> - initial-value prior:
#> - lognormal distribution:
#>   meanlog:
...
**Default:** Not used (GP is default). When enabled via `rt_opts(prior = RW(...))`, the prior step SD is `Normal(0, 0.1)` and the step interval is set with `period`.

**Expected impact of changes:**

- **Shorter steps (e.g., `RW(period = 1)`)**: $R_t$ can change daily. Very flexible, like a short GP length scale. Fast to compute but may overfit.
- **Longer steps (e.g., `RW(period = 7)` or `RW(period = 14)`)**: $R_t$ constant within each week/fortnight. Appropriate when transmission changes are discrete (policy changes) rather than continuous.

**When to modify:**

- Use random walk instead of GP when you believe transmission changes are discrete rather than smooth
- Use weekly (`RW(period = 7)`) when transmission dynamics change at weekly intervals
- Use daily (`RW(period = 1)`) for maximum flexibility with fast computation (but risk overfitting)
...
fit_rw <- estimate_infections(
  ...
  rt = rt_opts(prior = RW(init = LogNormal(mean = 2, sd = 0.5), period = 7)),
  stan = stan
)
...
**What it controls:** The `scale` parameter in `obs_opts()` is shared by both `estimate_infections()` and `estimate_secondary()`, but represents different quantities in each context:

- **For `estimate_infections()`**: The proportion of infections that are ultimately reported (ascertainment rate)
- **For `estimate_secondary()`**: The fraction of primary events leading to secondary events (e.g., case fatality rate, hospitalization rate)

**Default:** `Fixed(1)` for both functions

For `estimate_infections()`, this assumes all infections are reported. For `estimate_secondary()`, this assumes a 1:1 relationship between primary and secondary events (which is rarely realistic).
...
- **Tight prior (e.g., `Normal(mean=0.01, sd=0.005)`)**: Constrains estimates near prior (e.g., 1% case fatality rate), faster convergence
- **Wide prior (e.g., `Normal(mean=0.05, sd=0.05)`)**: Flexible but may be poorly identified without long time series
...
- **Always specify** based on domain knowledge (unlike `estimate_infections()`, the default of 1 is rarely appropriate)
- Use tighter priors with external estimates (e.g., infection fatality rate from serology studies)
...
fit_renewal <- estimate_infections(
  ...
  generation_time = gt_opts(example_generation_time),
  delays = delay_opts(delay),
  rt = rt_opts(prior = GP(init = LogNormal(mean = 2, sd = 0.5))),
  stan = stan
)
#> Warning: The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
...
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 1): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat - 
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 2): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat -
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 1): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess - 
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 2): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess -
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 1): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess - 
#> WARN [2026-06-18 10:46:42] estimate_secondary (chain: 2): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess -
...
#> WARN [2026-06-18 10:47:02] estimate_secondary (chain: 1): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat - 
#> WARN [2026-06-18 10:47:02] estimate_secondary (chain: 2): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat -
#> WARN [2026-06-18 10:47:03] estimate_secondary (chain: 1): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess - 
#> WARN [2026-06-18 10:47:03] estimate_secondary (chain: 2): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess -
#> WARN [2026-06-18 10:47:03] estimate_secondary (chain: 1): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess - 
#> WARN [2026-06-18 10:47:03] estimate_secondary (chain: 2): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess -
```
...
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 1): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat - 
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 2): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat -
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 1): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess - 
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 2): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess -
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 1): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess - 
#> WARN [2026-06-18 10:47:25] estimate_truncation (chain: 2): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess -
...
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 1): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat - 
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 2): The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#r-hat -
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 1): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess - 
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 2): Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#bulk-ess -
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 1): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess - 
#> WARN [2026-06-18 10:48:01] estimate_truncation (chain: 2): Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
#> Running the chains for more iterations may help. See
#> https://mc-stan.org/misc/warnings.html#tail-ess -
```
...
2. **Does $R_t$ change rapidly or gradually in my setting?** → Consider modifying the `ls` argument of `GP()`
...
estimates_r0 <- estimate_infections(
  ...
  generation_time = gt_opts(example_generation_time),
  delays = delay_opts(example_incubation_period + reporting_delay),
  rt = rt_opts(prior = GP(init = LogNormal(mean = 2.5, sd = 0.5))),
  stan = stan
)
#> Warning: The largest R-hat is NA, indicating chains have not mixed.
#> Running the chains for more iterations may help. See
...
#>  1 params[1]              0.994
#>  2 params[2]              1.01 
#>  3 initial_infections[1]  1.01 
#>  4 delay_params[1]        1.02 
#>  5 delay_params[2]        1.02 
#>  6 delay_params[3]        1.00 
#>  7 delay_params[4]        0.996
#>  8 day_of_week_simplex[1] 1.00 
#>  9 day_of_week_simplex[2] 1.16 
#> 10 day_of_week_simplex[3] 1.09 
#> # ℹ 588 more rows
...
#>    <chr>                     <dbl>    <dbl>
#>  1 params[1]                  96.5     77.8
#>  2 params[2]                  99.8     81.2
#>  3 initial_infections[1]     126.      52.5
#>  4 delay_params[1]            83.2     80.5
#>  5 delay_params[2]           104.      78.8
#>  6 delay_params[3]            89.0     78.1
#>  7 delay_params[4]           143.      80.5
#>  8 day_of_week_simplex[1]     88.6     53.8
#>  9 day_of_week_simplex[2]    131.      48.6
#> 10 day_of_week_simplex[3]     54.1     27.0
#> # ℹ 588 more rows
...
#>                         measure                 estimate
#>                          <char>                   <char>
#> 1:       New infections per day      2192 (1515 -- 3691)
#> 2:   Expected change in reports        Likely decreasing
#> 3:   Effective reproduction no.       0.88 (0.75 -- 1.1)
#> 4:               Rate of growth -0.032 (-0.084 -- 0.043)
#> 5: Doubling/halving time (days)         -21 (16 -- -8.2)
```
...
**Solution:** If you want daily flexibility, use a random walk (`rt_opts(prior = RW(period = 1))`) rather than GP with very short length scale. GPs with length scale <7 days often struggle.
