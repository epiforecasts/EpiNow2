...
```{r}
# You can examine prior choices using the distributional package
prior_r0 <- dist_lognormal(log(2), 0.5)
quantile(prior_r0, c(0.025, 0.5, 0.975))
# Check if this range is sensible for your application
```

Note: Full prior predictive checks (simulating complete epidemic trajectories) require additional Stan code not currently exposed in `{EpiNow2}`, but checking individual prior distributions as shown above is a useful first step.
