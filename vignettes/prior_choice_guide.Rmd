...
| Initial R₀ | Renewal only | `rt_opts()` | `prior` | `LogNormal(mean = 1, sd = 1)` | With stationary GP, reverts to this when no data |
| Random walk SD | Renewal only | `rt_opts()` | `rw` | `HalfNormal(0, 0.1)` | Set step size via `rw` argument |
| **Gaussian Process** |
...
| **Generation time** |
| Parameters | Renewal only | `gt_opts()` | `dist` | `Fixed(1)` | Default: daily exponential growth rate |
| **Delays** |
...
- **Stationary GP** (`gp_on = "R0"`): This is the prior mean that Rt reverts to when there's no data. Useful when you want estimates to return to a baseline value in the absence of information.
