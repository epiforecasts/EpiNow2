...
The following table lists all priors available in EpiNow2, organized by model component.

| Component | Function | Parameter | Default | Syntax Example |
|-----------|----------|-----------|---------|----------------|
| **Reproduction number (Rt)** |
| Initial R₀ | `rt_opts()` | `prior` | `LogNormal(mean=1, sd=1)` | `rt_opts(prior = LogNormal(mean=2, sd=0.5))` |
| Random walk SD | `rt_opts()` | (auto) | `HalfNormal(0, 0.1)` | Set via `rw` option: `rt_opts(rw=7)` |
| **Gaussian Process** |
| Length scale | `gp_opts()` | `ls` | `LogNormal(mean=21, sd=7, max=60)` | `gp_opts(ls = LogNormal(mean=14, sd=5, max=30))` |
| Magnitude | `gp_opts()` | `alpha` | `Normal(mean=0, sd=0.01)` | `gp_opts(alpha = Normal(mean=0, sd=0.05))` |
| **Observation model** |
| Overdispersion | `obs_opts()` | `dispersion` | `Normal(mean=0, sd=0.25)` | `obs_opts(dispersion = Normal(mean=0, sd=0.5))` |
| Scaling | `obs_opts()` | `scale` | `Fixed(1)` | `obs_opts(scale = Normal(mean=0.5, sd=0.1))` |
| Day of week | `obs_opts()` | (auto) | `Dirichlet(1,...,1)` | Turn off via `week_effect=FALSE` |
| **Generation time** |
| Parameters | `gt_opts()` | `dist` | Fixed at 1 day | `gt_opts(Gamma(shape=Normal(3,1), rate=Normal(2,0.5)))` |
| **Delays** |
| Parameters | `delay_opts()` | `dist` | Fixed at 0 days | `delay_opts(LogNormal(mean=5, sd=2, max=14))` |
| **Truncation** |
| Distribution | `trunc_opts()` | `dist` | Fixed at 0 days | `trunc_opts(LogNormal(mean=0.5, sd=0.5, max=3))` |
| **Initialization** |
| Initial infections | (internal) | `iota` | `Normal(log(C_init), 2)` | Not user-adjustable |
