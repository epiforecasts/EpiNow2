...
- **`weight_prior` option**: Default `TRUE` weights the prior by data length, keeping parameters near their priors. Set `FALSE` to allow more learning from data, though in practice this often doesn't work well.
...
- **`weight_prior = FALSE`** can be tried if you believe the generation time should be learned from data, though this is rarely successful in practice
