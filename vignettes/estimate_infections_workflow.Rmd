...
``` r
plot(Gamma(mean = 3, sd = 1, max = 10))
```

![plot of chunk unnamed-chunk-9](estimate_infections_workflow-unnamed-chunk-9-1.png)
...
#> Warning: ! Uncertain gamma distribution specified in terms of parameters that are not the
#>   "natural" parameters of the distribution shape and rate.
#> ℹ Converting using a crude and very approximate method that is likely to produce
#>   biased results.
#> ℹ If possible it is preferable to specify the distribution directly in terms of the
#>   natural parameters.
#> - gamma distribution (max: 10):
#>   shape:
#>     - normal distribution:
...
![plot of chunk unnamed-chunk-11](estimate_infections_workflow-unnamed-chunk-11-1.png)
...
``` r
plot(incubation_period + reporting_delay)
```

![plot of chunk unnamed-chunk-14](estimate_infections_workflow-unnamed-chunk-14-1.png)
...
#> DEBUG [2024-09-19 14:46:51] estimate_infections: Running in exact mode for 2000 samples (across 4 chains each with a warm up of 250 iterations each) and 147 time steps of which 7 are a forecast
