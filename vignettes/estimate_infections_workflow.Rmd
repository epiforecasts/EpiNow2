...
## Reproduction number prior

The default model that `estimate_infections()` uses to estimate reproduction numbers requires specification of a prior probability distribution for the reproduction number.
With the default non-stationary Gaussian process the cumulated GP is mean-centred so this prior is on the mean reproduction number over the observation window; with no GP or with the stationary GP it is the prior on the initial reproduction number.
It represents the user's prior belief of the central value of the reproduction number, before any data informs it.
