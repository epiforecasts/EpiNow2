...
#> 
#>   distribution with PMF [0.0038 0.15 0.39 0.3 0.12 0.03 0.006 0.00096 0.00013 1.6e-05 1.8e-06].
```
...
#> Warning in generate_dist_spec(params, "gamma"): Uncertain gamma distribution
...
#> to specify the distribution direclty in terms of the natural parameters.
#> 
#>   gamma distribution (max: 10) with uncertain shape (mean = 9, sd = 3.4) and uncertain rate (mean = 3, sd = 1.2).
```
...
In _EpiNow2_, the generation time distribution is defined by a call to `generation_time_opts()`, a function that takes a single argument defined as a `dist_spec` object (returned by the function corresponding to the probability distribution, i.e. `lognormal()`, `gamma()`, etc.).
...
generation_time <- gamma(
  shape = normal(9, 3.4), rate = normal(3, 1.2), max = 10
)
...
#>   lognormal distribution (max: 14) with uncertain meanlog (mean = 1.6, sd = 0.05) and uncertain sdlog (mean = 0.5, sd = 0.05).
#>   distribution with PMF [0.16 0.49 0.23 0.077 0.025 0.0084 0.003 0.0011 0.00045 0.00019 8.2e-05].
```
...
#> Warning: There were 1000 transitions after warmup that exceeded the maximum treedepth. Increase max_treedepth above 15. See
