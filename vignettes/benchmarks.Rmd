...
|model                      |description                                                                      |
|:--------------------------|:--------------------------------------------------------------------------------|
|default_mcmc               |Default model (non-stationary prior on $R_t$); fitting withmcmc                  |
|default_vb                 |Default model (non-stationary prior on $R_t$); fitting withvariational bayes     |
|default_pathfinder         |Default model (non-stationary prior on $R_t$); fitting withpathfinder algorithm  |
|default_laplace            |Default model (non-stationary prior on $R_t$); fitting withlaplace approximation |
|non_mechanistic_mcmc       |no mechanistic prior on $R_t$; fitting withmcmc                                  |
|non_mechanistic_vb         |no mechanistic prior on $R_t$; fitting withvariational bayes                     |
|non_mechanistic_pathfinder |no mechanistic prior on $R_t$; fitting withpathfinder algorithm                  |
|non_mechanistic_laplace    |no mechanistic prior on $R_t$; fitting withlaplace approximation                 |
|rw7_mcmc                   |7-day random walk prior on $R_t$; fitting withmcmc                               |
|rw7_vb                     |7-day random walk prior on $R_t$; fitting withvariational bayes                  |
|rw7_pathfinder             |7-day random walk prior on $R_t$; fitting withpathfinder algorithm               |
|rw7_laplace                |7-day random walk prior on $R_t$; fitting withlaplace approximation              |
|non_residual_mcmc          |Stationary prior on $R_t$; fitting withmcmc                                      |
|non_residual_vb            |Stationary prior on $R_t$; fitting withvariational bayes                         |
|non_residual_pathfinder    |Stationary prior on $R_t$; fitting withpathfinder algorithm                      |
|non_residual_laplace       |Stationary prior on $R_t$; fitting withlaplace approximation                     |
...
# Re-categorise the four(4) fitting methods into either MCMC or approximate.
...
process_crps <- function(results, variable, truth) {
  ...
  crps_with_dates <- purrr::map_depth(
    ...
    2,
    ~ data.table(crps = .x)[,
      date := min(infections_true$date) + 0: (.N - 1)
    ]
  )
...
The approximate fitting methods (vb, pathfinder, and laplace) are the fastest, irrespective of the model. For the mcmc fitted models, the default model is the slowest across all the data scenarios. The non-mechanistic model is the fastest among them and even almost as fast as the models fitted with the approximate methods.

### Evaluating model performance

Now, we will evaluate the performance of the models. We will look at the total and time-varying performance of the models using the [continuous ranked probability score (CRPS)](https://en.wikipedia.org/wiki/Scoring_rule#Continuous_ranked_probability_score). CRPS is a [proper scoring rule](https://en.wikipedia.org/wiki/Scoring_rule#Propriety_and_consistency) that measures the accuracy of probabilistic forecasts. When comparing models, the smaller the CRPS, the better.

To evaluate $R_t$ estimation performance, we will compare the final estimated value of $R_t$ for each model, as this represents the nowcasting performance of the models. For the infections, we will compare the 7-day forecast.
...
plot_crps_over_time <- function(data, title) {
    plot <- ggplot(data = data[!is.na(crps)]) + # remove failed models
        ...
        guides(
            ...
            linetype = guide_legend(title = "Fitting")
        ) +
...
rt_crps_plot <- plot_crps_over_time(rt_crps_dt_final, "Time-varying model performance in estimating Rt")
rt_crps_plot  +
...
infections_crps_plot <- plot_crps_over_time(infections_crps_dt_final, "Time-varying model performance in estimating infections")
...
plot_total_crps <- function(data, title) {
  plot <- ggplot(data = data) +
    ...
    guides(
      color = guide_legend(title = "Fitting")
    ) +
    labs(
      x = "Model",
      y = "Total CRPS",
...
``` r
# Calculate
rt_total_crps <- calculate_total_crps(rt_crps_dt_final, by = c("model_basename", "fitting", "epidemic_phase"))

# Process data and plot
rt_total_crps_plot <- plot_total_crps(rt_total_crps, "Aggregated model performance in estimating Rt")
rt_total_crps_plot +
  # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_flip() +
  facet_wrap(~epidemic_phase, strip.position = "bottom")
```
...
``` r
# Calculate
infections_total_crps <- calculate_total_crps(infections_crps_dt_final, by = c("model_basename", "fitting", "epidemic_phase"))

# Process and plot
infections_total_crps_plot <- plot_total_crps(infections_total_crps, "Aggregated model performance in estimating infections")
infections_total_crps_plot +
  scale_y_continuous(label = scales::label_number(scale = 1/10000)) +
  scale_shape_manual(values = c(0, 1, 17, 8)) +
  coord_flip() +
  labs(y = "total CRPS (x 10k)") +
  facet_wrap(~epidemic_phase, strip.position = "bottom")
```
...
rt_now_comparison_plot <- ggplot(data = rt_nowcast) +
    ...
    guides(color = guide_legend(title = "Fitting type")) +
    labs(title = "Nowcasting performance (Rt)",
         x = "Model",
         caption = plot_caption_custom
         ) + 
    facet_wrap(~epidemic_phase, strip.position = "bottom") +
...
infections_real_time_comparison_plot <- ggplot(data = infections_real_time) +
    ...
    guides(color = guide_legend(title = "Fit type")) +
    labs(title = "Real-time performance (infections)",
         y = "crps (x 10k)",
         x = "Model",
         caption = plot_caption_custom
    ) + 
...
In terms of run times, the mcmc-fitted models are the slowest. The default model is especially slowest, followed by the non-residual model, and the 7-day random walk model. The non-mechanistic model is almost as fast as the non-mcmc models.

When we look at the overall performance, ...

For $R_t$ nowcasting performance, ...

Finally, for real-time infection forecast performance
