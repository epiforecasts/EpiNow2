...
#>      <char>                 <char>                     <fctr>
#> 1: realland    2242 (1403 -- 3693)          Likely decreasing
#> 2: testland    2222 (1361 -- 3627)          Likely decreasing
#>    Effective reproduction no.           Rate of growth
#>                        <char>                   <char>
#> 1:         0.89 (0.72 -- 1.1) -0.028 (-0.091 -- 0.039)
#> 2:         0.89 (0.71 -- 1.1)  -0.03 (-0.093 -- 0.038)
...
If instead, we wanted to use the Gaussian process for `testland` and a random walk for `realland` we could specify these separately using the `opts_list()` function from the package.


``` r
rt <- opts_list(
  rt_opts(), cases,
  realland = rt_opts(prior = RW(init = LogNormal(mean = 1, sd = 1), period = 7))
)
...
#>      Region New infections per day Expected change in reports
#>      <char>                 <char>                     <fctr>
#> 1: realland    2019 (1060 -- 3792)          Likely decreasing
#> 2: testland    2247 (1399 -- 3661)          Likely decreasing
#>    Effective reproduction no.           Rate of growth
#>                        <char>                   <char>
#> 1:          0.85 (0.6 -- 1.2)  -0.041 (-0.11 -- 0.042)
#> 2:         0.89 (0.72 -- 1.1) -0.029 (-0.089 -- 0.035)
#>    Doubling/halving time (days)
#>                          <char>
#> 1:             -17 (17 -- -6.1)
#> 2:             -24 (20 -- -7.8)
