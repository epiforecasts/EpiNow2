...
A prior predictive check helps verify that the priors imply a plausible range of delay distributions.
We draw from the priors and plot the implied densities.
...
prior_densities <- sapply(seq_len(n_draws), function(i) {
  ml <- rnorm(1, 1.6, 0.5)
  sl <- max(rnorm(1, 0.5, 0.25), 0.1)
  dlnorm(x_seq, ml, sl)
})

prior_df <- data.frame(
  x = rep(x_seq, n_draws),
  density = as.vector(prior_densities),
  draw = rep(seq_len(n_draws), each = length(x_seq))
)

ggplot(prior_df, aes(x = x, y = density, group = draw)) +
  geom_line(alpha = 0.2, colour = "#4292C6") +
  labs(
    x = "Delay (days)", y = "Density",
    title = "Prior predictive: implied delay distributions"
  ) +
...
We pass the linelist and our custom priors to `estimate_dist()`.
...
#> 1:  meanlog 1.4030597 1.4133233 0.09858172 1.2723868 1.3489197 1.3820042
#> 2:    sdlog 0.7882222 0.7953719 0.07807894 0.6788305 0.7423436 0.7706853
#>     upper_20 upper_50  upper_90
#>        <num>    <num>     <num>
#> 1: 1.4258920 1.465487 1.5843156
#> 2: 0.8080633 0.841584 0.9290615
```
...
#> params[1]          1.4133233 0.004316969 0.09858172    1.2518316    1.3489197
#> params[2]          0.7953719 0.003063368 0.07807894    0.6636291    0.7423436
#> delay_params[1]    1.4133233 0.004316969 0.09858172    1.2518316    1.3489197
#> delay_params[2]    0.7953719 0.003063368 0.07807894    0.6636291    0.7423436
#> lp__            -375.0461582 0.052024836 1.12104595 -378.0435964 -375.4796121
#>                          50%         75%        97.5%    n_eff     Rhat
#> params[1]          1.4030597    1.465487    1.6417671 521.4767 1.000459
#> params[2]          0.7882222    0.841584    0.9624373 649.6350 1.002513
#> delay_params[1]    1.4030597    1.465487    1.6417671 521.4767 1.000459
#> delay_params[2]    0.7882222    0.841584    0.9624373 649.6350 1.002513
#> lp__            -374.7006841 -374.270796 -374.0016991 464.3286 1.013132
```
