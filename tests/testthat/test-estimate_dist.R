...
test_that(".prepare_interval_data converts vector correctly", {
  delays <- c(1, 2, 2, 3, 3, 3, 5)

  result <- .prepare_interval_data(delays, verbose = FALSE)
...
test_that(".prepare_interval_data handles NA and negative values", {
...
test_that(".get_param_bounds returns sensible bounds", {
  ...
  # Lognormal
  bounds_ln <- .get_param_bounds(delay_data, "lognormal")
