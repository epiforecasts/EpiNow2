...
test_that("estimate_secondary warns when delay PMF exceeds data length", {
  ...
  # Create short secondary observation data (but long enough for burn_in)
  secondary_data <- data.table::copy(EpiNow2::example_confirmed[1:20])[
    ,
    `:=`(
      primary = confirm,
...
test_that("estimate_secondary warns when combined delay PMFs exceed data length", {
  rlang::local_options(rlib_warning_verbosity = "verbose")
  
  # Short secondary data (but long enough for burn_in)
  secondary_data <- data.table::copy(EpiNow2::example_confirmed[1:20])[
    ,
    `:=`(
      primary = confirm,
  ...
  # Two medium-length PMFs that combine to exceed data
  delay1 <- NonParametric(rep(0.2, 5))  # length 5
...
test_that("estimate_secondary runs without PMF warnings when lengths are appropriate", {
  rlang::local_options(rlib_warning_verbosity = "verbose")
  
  # Adequate data length
  secondary_data <- data.table::copy(EpiNow2::example_confirmed[1:30])[
    ,
    `:=`(
      primary = confirm,
      secondary = round(0.5 * confirm),
  ...
  # Short PMF that won't trigger warnings
