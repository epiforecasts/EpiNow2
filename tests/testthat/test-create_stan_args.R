...
test_that("create_stan_args returns the expected defaults when the exact method is used", {
  ...
  expect_equal(names(create_stan_args()), c("object", "method", "data", "init", "refresh", "cores",
                                            "warmup", "chains", "control", "save_warmup", "seed",
                                            "iter"))
})

test_that("create_stan_args returns the expected defaults when the approximate method is used", {
  
  expect_equal(names(create_stan_args(stan_args = list(method = "vb"))), c("object", "method", "data", 
                                                                           "init", "refresh", "trials", 
                                                                           "iter", "output_samples"))
})
