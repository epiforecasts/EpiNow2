...
- The
  ...
  function now uses the `primarycensored` package for double censored
  PMF calculations, replacing the previous CDF difference approximation.
...
- Updated DESCRIPTION: removed “EpiForecasts” from authors, revised
  ...
  dependencies.
- Fixed integration tests for `estimate_truncation` and
  `estimate_secondary` parameter recovery, and updated
  `example_truncated` data generation to use the Stan `discretised_pmf`
  function directly.
- Updated
...
- Development-only dependencies (`covr`, `here`, `hexSticker`, `magick`,
  ...
  all dependencies including dev tools. Fixes
  [\#1067](https://github.com/epiforecasts/EpiNow2/issues/1067).
...
- [`estimate_infections()`](https://epiforecasts.io/EpiNow2/dev/reference/estimate_infections.md)
  ...
  - **Deprecated**: Internal function
    [`extract_parameter_samples()`](https://epiforecasts.io/EpiNow2/dev/reference/extract_parameter_samples.md)
    renamed to
...
- [`estimate_secondary()`](https://epiforecasts.io/EpiNow2/dev/reference/estimate_secondary.md)
  ...
  - Use `get_predictions(object)` to get predicted secondary
    observations with credible intervals merged with observations.
...
- Refactored
  ...
  - Use `get_predictions(object)` to extract truncation-adjusted
    (nowcast) estimates that can be compared to observed data.
...
- Added an option to compute growth rates using an estimator by Parag et
  ...
  see `growth_method` argument in rt_opts().
...
- A bug was fixed where the `report_log_lik` Stan function used the raw
  overdispersion parameter instead of the transformed phi value,
  producing incorrect pointwise log-likelihood values for model
  comparison (LOO, WAIC).
- A bug was fixed where the truncation PMF vector in
  `estimate_secondary.stan` was declared with incorrect dimension,
  causing a dimension mismatch with the `get_delay_rev_pmf()` function
  call.
- A bug was fixed where the `CrIs` parameter in
  [`epinow()`](https://epiforecasts.io/EpiNow2/dev/reference/epinow.md)
  was not being passed through to internal functions, causing
  user-specified credible intervals to be ignored in saved files and
  output.
- A bug was fixed where `forecast_infections` would fail with
  `samples = 1`.
- A bug was fixed where
  [`opts_list()`](https://epiforecasts.io/EpiNow2/dev/reference/opts_list.md)
  recursed lists which it shouldn’t.
- A bug was fixed where shifted cases for the deconvolution model did
  not reflect accumulation settings.
- A bug was fixed where `estimate_infection()` threw an error if there
  were too many consecutive `NA` observations.
- A bug was fixed where an error was thrown when convolving delay
  distributions with very small values.
- A bug was fixed where various log variables (time steps, horizon,
  samples, chains, and iterations) were not being reported correctly
  especially when using the `cmdstanr` backend and `estimate_delays()`
  function.
- A bug was fixed where a cli warning was broken due to bad syntax.
- A bug was fixed where intermediate data was being bound together by
  column position instead of column name, leading to erroneous results.
- A bug was fixed in the implementation of the Matern 5/2 Gaussian
  process kernel spectral density. See discussion at
  <https://discourse.mc-stan.org/t/diagspd-for-matern-gp-kernels-other-than-3-2/37011/12>
...
- The stan code is now fulled documented and can be accessed on the
  website under the Reference tab.
...
- All functions now use the [cli](https://cli.r-lib.org) R package to
  ...
  reviewed by [@seabbs](https://github.com/seabbs).
- [`fix_dist()`](https://epiforecasts.io/EpiNow2/dev/reference/fix_dist.md)
  has been renamed to
