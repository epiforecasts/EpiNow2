...
Alternatively, `cdf_max` sets the truncation point at the value where
the CDF reaches a given level (e.g. `cdf_max = 0.999` keeps the
distribution up to its 99.9th percentile).
...
point and renormalising. Alternatively, `cdf_max` sets the truncation
point at the value where the CDF reaches a given level.
