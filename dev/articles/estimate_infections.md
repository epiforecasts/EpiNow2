...
$$I_{t} = R_{t}\sum\limits_{\tau = 1}^{g_{max}}g\left( \tau|\theta_{g} \right)I_{t - \tau}$$

where $g_{\tau} = g\left( \tau|\theta_{g} \right)$ is the discretised
distribution of generation times with parameters $\theta_{g}$ and
maximum $g_{max}$. Generation times can be specified as coming from a
distribution with uncertainty by giving mean and standard deviations of
normal priors of the distributional parameters. By default this prior is
weighted by default by the number of observations although this can be
changed by the user. It is truncated to be positive where relevant for
the given distribution. Alternatively, generation times can be specified
as coming from a given distribution with set parameters.
...
options. These included fixing transmission dynamics (optionally this
can also be done before the end of the observation period), and scaling
$R_{t}$ based on the proportion of the population that remain
susceptible. This is defined as followed,

$$I_{t} = \left( N - I_{t - 1}^{c} \right)\left( 1 - \exp\left( \frac{- I\prime_{t}}{N - I_{T}^{c}} \right) \right),$$

where $I_{t}^{c} = \sum_{s < t}I_{s}$ are cumulative infections by
$t - 1$ and $I\prime_{t}$ are the unadjusted infections defined above.
This adjustment is based on the one implemented in the `epidemia` R
package^(\[2\]).
...
Scott, J. A. (n.d.). *Semi-Mechanistic Bayesian modeling of COVID-19
with Renewal Processes*. 14.
