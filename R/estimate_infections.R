...
#' Gaussian process. Defaults to [gp_opts()]. Used by the back-calculation
#' model (`rt = NULL`) to define the Gaussian process on latent infections.
...
estimate_infections <- function(data,
  ...
  # and I a Gaussian process on log-infections anchored at an initial value I0.
  # Each state's GP carries its own hyperparameters, so there are no separate
  # alpha/rho parameters and no shared "main" GP.
  renewal <- isTRUE(rt$use_rt)
  i0_guess <- mean(head(data$confirm[!is.na(data$confirm)], 7))
  if (!is.finite(i0_guess) || i0_guess <= 0) {
    i0_guess <- 1
  }
  i_prior <- new_state_spec(
    "gp", init = LogNormal(mean = i0_guess, sd = i0_guess),
    settings = if (is.null(gp)) gp_opts() else gp
  )
  params <- list(
    make_param("R", if (renewal) rt$prior else NULL, lower_bound = 0),
    make_param("I", if (renewal) NULL else i_prior, lower_bound = 0),
