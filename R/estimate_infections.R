...
#' `confirm` must be non-negative numeric and `date` must be in date format
#' with no missing (`NA`) entries; data that does not meet these requirements
#' is rejected with an informative error before model fitting starts.
...
#' [fill_missing()] function which helps add the `accumulate` column with the
#' desired properties when dealing with non-daily data. If any accumulation is
...
#' Passing anything else, such as a bare `<dist_spec>` rather than the result
#' of a call to [gt_opts()], is rejected with an error before model fitting
#' starts.
#'
...
#' details. This must be the result of a call to [delay_opts()]; anything
...
#' in the estimate. This must be the result of a call to [trunc_opts()];
