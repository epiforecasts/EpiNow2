...
#' @importFrom cli cli_abort
#' @return A `<forecast_infections>` object (which inherits from
#' `<estimate_infections>`) containing output as returned by [estimate_infections()]
#' but based on results from the specified scenario rather than fitting.
...
forecast_infections <- function(estimates,
  ...
  class(format_out) <- c("forecast_infections", "estimate_infections", class(format_out))
  return(format_out)
}
