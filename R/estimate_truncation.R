...
#' the data.
#'
#' 2. The data set with the latest observations is adjusted for
#' truncation using the truncation distribution.
#'
...
#' of the reported data over time. Data sets are expected to differ in
#' length because later snapshots include more recently reported dates; this
#' difference in length is exactly the truncation signal the model is
#' estimating from. All data sets must, however, contain a complete vector of
#' dates over the range they do cover, since dates missing from a snapshot are
#' treated as not yet reported rather than validated for gaps. This is not
#' currently checked, so a data set with a date accidentally missing within
#' its range will silently bias the truncation estimate rather than being
#' rejected.
#'
#' @param obs A list of observation model options as generated by
#' [obs_opts()]. The truncation model uses `family`, `dispersion`,
#' `likelihood` and `return_likelihood`. Other settings (`weight`,
#' `week_effect`, `scale`) are ignored, since week effects and scaling
#' are not modelled here. Defaults to [obs_opts()].
#'
...
estimate_truncation <- function(data,
  ...
  stan_data <- list(
    ...
    likelihood = obs_model$likelihood,
    return_likelihood = obs_model$return_likelihood
  )
