...
##' Set backend-specific options for fixed parameter sampling
##'
##' Each backend asks for sampling from fixed parameters in a different way.
##' `rstan` takes an algorithm name, `cmdstanr` takes a flag, and `stanli`
##' selects it itself for models without parameters. The adaptation settings
##' are dropped where they do not apply.
##'
##' @param stan A list of stan options as generated by [stan_opts()].
##' @return The stan options with fixed parameter settings applied.
##' @keywords internal
set_fixed_param_opts <- function(stan) {
  switch(stan$backend,
    rstan = {
...
create_stan_args <- function(stan = stan_opts(),
                             ...
                             verbose = FALSE) {
  if (fixed_param) {
    stan <- set_fixed_param_opts(stan)
  }
  ## generate stan model
  ...
  # zero-size ones that cannot be given one, so the initial conditions
  # generated elsewhere in the package cannot be used with it. Random
  # initialisation is used instead, with the radius given by init_radius.
  if (inherits(stan$object, "stanli_cstanmodel")) {
    if (!is.character(init) || init != "random") {
      cli_inform(
        c("i" = "Using random initialisation with the {col_blue('stanli')}
          backend, which does not support the initial conditions
          {.pkg EpiNow2} generates."),
        .frequency = "once",
        .frequency_id = "stanli-random-init"
      )
    }
    init <- NULL
  }
