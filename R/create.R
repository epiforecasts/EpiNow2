...
##' `rstan` takes an algorithm name while the others take a flag. The
##' adaptation settings are dropped where they do not apply.
##'
...
create_stan_args <- function(stan = stan_opts(),
  ...
  # neither cmdstanr nor stanr have an init = "random" argument, taking the
  # radius to draw random initial values within instead
  if (is.character(init) && init == "random" &&
        (inherits(stan$object, "CmdStanModel") ||
           inherits(stan$object, "StanModel"))) {
    init <- 2
  }
