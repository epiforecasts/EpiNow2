...
Some AI tools can automatically [co-author git commits](https://docs.github.com/en/pull-requests/committing-changes-to-your-project/creating-and-editing-commits/creating-a-commit-with-multiple-authors), but if that's not the case for the tool you use, ensure you acknowledge AI contributions in a transparent way. Typical ways to indicate this apart from co-authorship by AI agents are co-authorship by a bot user (useful if wanting to avoid reference to the particular model used) or a mention of "Generated by". We leave it up to contributors to choose their preferred way of doing this.
