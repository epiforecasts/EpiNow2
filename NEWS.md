...
- `estimate_truncation()` now returns a proper S3 object with a simplified structure.
  ...
  - Removed elements: `last_obs` (now included in `observations`), `cmf` and `dist` (use `get_dist(object)` to extract the truncation distribution as a `dist_spec`).
  - Use `get_dist(object)` to extract the estimated truncation distribution for use in `epinow()` or `estimate_infections()`.
  ...
  - **Deprecated**: Accessing `$dist` via `$` or `[[` triggers deprecation warnings. Use `get_dist()` instead.
