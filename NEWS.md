...
- `estimate_infections()` now accepts a `model` argument to explicitly specify the infection model via `renewal_opts()` or `deconvolution_opts()`.
  ...
  - Use `model = renewal_opts(prior = ..., generation_time = ...)` for the renewal equation model.
  - Use `model = deconvolution_opts(prior = ..., prior_window = ...)` for the deconvolution model (previously selected using `rt = NULL`).
  - The new functions incorporate all relevant parameters directly without nesting.
