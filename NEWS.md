...
- The `variable` column in `get_samples()` and `summary()` output now contains semantic parameter names (e.g., `"generation_time[1]"`, `"reporting_overdispersion"`) instead of generic category names like `"delay_params"` or `"params"`. Existing code filtering by `variable == "R"` or similar continues to work unchanged.
