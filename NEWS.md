...
- Added experimental `GP()` and `RW()` constructors for declaring time-varying parameters, and wired up time-varying ascertainment via `obs_opts(scale = RW(...))` or `obs_opts(scale = GP(...))` in `estimate_infections()`.
