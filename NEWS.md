...
- The `default_cdf_cutoff` argument of `gt_opts()`, `delay_opts()` and `trunc_opts()` has been renamed to `default_cdf_max`, following `distspec`'s renamed `cdf_max` argument (EpiNow2 now requires `distspec` >= 0.2.0). It is the CDF level to keep the distribution up to (default `0.999`), where `1` leaves it unbounded; the previous convention was the tail probability to remove (default `0.001`). The default can be set globally with `options(EpiNow2.cdf_max = ...)`. `default_cdf_cutoff` is deprecated.
