...
- Added experimental `GP()` and `RW()` constructors for declaring time-varying parameters. In `estimate_infections()` the observation scaling (`obs_opts(scale = ...)`) and reporting overdispersion (`obs_opts(dispersion = ...)`) can now be made time-varying with a random walk or Gaussian process.
