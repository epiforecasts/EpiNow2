...
- Fixed a bug where `estimate_dist()` would fail with a "model fitting timed out or failed" error when all observed delays were identical. Scale parameters are now initialised from the prior when variance cannot be estimated from the data, and a warning is issued.
